...
bool TCPServer::Start()
{
	...
	messages = new SafeQueue<TCPPacket>();

	// Start listening for incoming packets on a new thread
	running = true;
	listen_thread = new std::thread(&TCPServer::Listen, this);

	return true;
}

bool TCPServer::Send_to_client(int socket, char* message, int length)
{
	return send(socket, message, length, 0) == length;
}
bool TCPServer::Send_to_all(char* message, int length, int socket_excluded)
{
	bool success = true;
		...
		for (i = 0; i < client_sockets.size(); i++)
		{
			...
			if (FD_ISSET(sd, &readfds))
			{
				...
				if ((valread = read(sd, buffer, 1024)) == 0)
				{
					...
					messages->push({ sd, TCPPacketType::Disconnect, nullptr, 0 });
				...
				else
				{
					if (tempBuffers.find(sd) == tempBuffers.end()) {
						...
						if (len <= valread - 2) {
							...
							messages->push({ sd, TCPPacketType::Message, buffer, valread });
						...
						char* new_buffer = new char[packetBuffer];
...
void TCPServer::Handle_new_connection()
{
	...
	messages->push({ new_socket, TCPPacketType::Connect, nullptr, 0 });
}
